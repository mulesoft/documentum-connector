...
public class AccessControlClientImpl implements AccessControlClient {
    ...
    public Acl createAcl(String aclName, String aclDescription, List<AclEntry> aclEntries, AclVisibility aclVisibility, AclType aclType) throws ServiceException,
        ...
        return port
                .create(createAclPackage(createAcl(createAclIdentity(Utils.getRepositoryName(serviceContext), Utils.getUserName(serviceContext), aclName), aclDescription, aclType,
                        aclVisibility, aclEntries))).getAcls().get(0);
    }
    ...
    public Acl updateAcl(String aclName, String aclDescription, List<AclEntry> aclEntries, AclVisibility aclVisibility, AclType aclType) throws ServiceException,
            CoreServiceException_Exception {
        return port
                .update(createAclPackage(createAcl(createAclIdentity(Utils.getRepositoryName(serviceContext), Utils.getUserName(serviceContext), aclName), aclDescription, aclType,
                        aclVisibility, aclEntries))).getAcls().get(0);
    }
    ...
    public AclPackage getAcls(QueryClient queryClient) throws ServiceException, SerializableException, CoreServiceException_Exception {
        return port.get(createAclIdentityList(getPropertiesValues(
                queryClient.query(("select owner_name, object_name from dm_acl where owner_name='" + Utils.getUserName(serviceContext) + "'")).getDataPackage().getDataObjects(),
                "object_name")));
    }
