...
import org.mule.modules.documentum.processors.CheckoutMessageProcessor;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            logger.error(("Cannot launch the mule app, the @Processor [checkout] within the connector [documentum] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [checkout] within the connector [documentum] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [checkout] within the connector [documentum] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
