...
import com.emc.documentum.fs.datamodel.core.acl.holders.AclIdentityExpressionHolder;
...
import org.mule.modules.documentum.processors.ApplyAclMessageProcessor;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            logger.error(("Cannot launch the mule app, the @Processor [apply-acl] within the connector [documentum] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [apply-acl] within the connector [documentum] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [apply-acl] within the connector [documentum] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
        parseConfigRef(element, builder);
        if (!parseObjectRef(element, builder, "object-identity", "objectIdentity")) {
        ...
        if (!parseObjectRef(element, builder, "acl-identity", "aclIdentity")) {
            BeanDefinitionBuilder aclIdentityBuilder = BeanDefinitionBuilder.rootBeanDefinition(AclIdentityExpressionHolder.class.getName());
            Element aclIdentityChildElement = DomUtils.getChildElementByTagName(element, "acl-identity");
            if (aclIdentityChildElement!= null) {
                parseProperty(aclIdentityBuilder, aclIdentityChildElement, "domain", "domain");
                parseProperty(aclIdentityBuilder, aclIdentityChildElement, "name", "name");
                parseProperty(aclIdentityBuilder, aclIdentityChildElement, "repositoryName", "repositoryName");
                builder.addPropertyValue("aclIdentity", aclIdentityBuilder.getBeanDefinition());
            }
        }
